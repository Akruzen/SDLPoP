...
int __pascal far play_kid_frame() {
	...
	if (Char.room != 0) {
		if ((current_level == 9 && curr_room == 11 && game_is_rewinding))
		{
			if (rewind_timer != 0)
			{
				Char.frame = rewind_frame_array[(rewind_timer - 1)];
				Char.x = rewind_XPos_array[(rewind_timer - 1)];
				Char.y = rewind_YPos_array[(rewind_timer - 1)];
				Char.direction = rewind_Kid_Direction[(rewind_timer - 1)];
			}
			else
			{
